...
module.exports = {
    ...
    execute(interaction,componentConfig,callback){
        ...
        if(session.type == "townVisit"){
            if(session.session_data.player.tutorial == 6 && interaction.values[0].split("_")[0] == "lives" && parseInt(interaction.values[0].split("_")[1]) == 2){
                session.session_data.player.gold = 0
                session.session_data.player.lives = 3
                session.session_data.player.tutorial = 7
            ...
            } else if(session.session_data.player.tutorial != "completed"){
                interaction.reply({ content: "As you are still in the tutorial, you must buy 2 slices of cake to raise your life total to 3", ephemeral: true });
            } else {
